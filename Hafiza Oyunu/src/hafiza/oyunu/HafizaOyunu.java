
package hafiza.oyunu;

import java.util.*;

public class HafizaOyunu {

    public static Kart[][] kartlar = new Kart[4][4];
    
    public static void oyuntahtasi(){
        System.out.println("____________________");
    for(int i = 0;i<4;i++){
    for(int j = 0;j<4;j++){
        if(kartlar[i][j].isTahmin()){
            System.out.print(" |"+kartlar[i][j].getDeger()+"| ");
           }
        else{
            System.out.print(" | | ");
        }
        }
    System.out.println(); 
    System.out.println("____________________");
    }
    }
    
    public static boolean oyunBittiMi(){
    for(int i = 0;i<4;i++){
    for(int j = 0;j<4;j++){
    if(kartlar[i][j].isTahmin()==false){
    
    return false;
    }
    
    }}
    return true;
    }
    
    public static void tahminEt(){
     Scanner s = new Scanner(System.in);
        System.out.println("Birinci Tahmin (ilk satırı,sonra sütunu giriniz.): ");
        int i1 = s.nextInt();
        int j1 = s.nextInt();
    kartlar[i1-1][j1-1].setTahmin(true);
    oyuntahtasi();
        System.out.println("İkinci Tahmin (ilk satırı,sonra sütunu giriniz.): ");
        int i2 = s.nextInt();
        int j2 = s.nextInt();
        kartlar[i2-1][j2-1].setTahmin(true);
        oyuntahtasi();
        if( kartlar[i1-1][j1-1].getDeger() == kartlar[i2-1][j2-1].getDeger()){
         kartlar[i2-1][j2-1].setTahmin(true);
            System.out.println("Tebrikler!");
        }
        else{
        kartlar[i2-1][j2-1].setTahmin(false);
        kartlar[i1-1][j1-1].setTahmin(false);
        }
        }
    
    public static void main(String[] args) {
       
        Scanner s = new Scanner(System.in);
        int seviye = 0;
        while(seviye!=6){
        switch(seviye){
        case 0:
        kartlar[0][0] = new Kart('E');
        kartlar[0][1] = new Kart('A');
        kartlar[0][2] = new Kart('B');
        kartlar[0][3] = new Kart('F');
        kartlar[1][0] = new Kart('G');
        kartlar[1][1] = new Kart('A');
        kartlar[1][2] = new Kart('D');
        kartlar[1][3] = new Kart('H');
        kartlar[2][0] = new Kart('F');
        kartlar[2][1] = new Kart('C');
        kartlar[2][2] = new Kart('D');
        kartlar[2][3] = new Kart('H');
        kartlar[3][0] = new Kart('E');
        kartlar[3][1] = new Kart('G');
        kartlar[3][2] = new Kart('B');
        kartlar[3][3] = new Kart('C');
        case 1:
        kartlar[0][0] = new Kart('D');
        kartlar[0][1] = new Kart('B');
        kartlar[0][2] = new Kart('E');
        kartlar[0][3] = new Kart('A');
        kartlar[1][0] = new Kart('C');
        kartlar[1][1] = new Kart('G');
        kartlar[1][2] = new Kart('H');
        kartlar[1][3] = new Kart('G');
        kartlar[2][0] = new Kart('A');
        kartlar[2][1] = new Kart('F');
        kartlar[2][2] = new Kart('D');
        kartlar[2][3] = new Kart('H');
        kartlar[3][0] = new Kart('B');
        kartlar[3][1] = new Kart('E');
        kartlar[3][2] = new Kart('C');
        kartlar[3][3] = new Kart('F');
        case 2:
        kartlar[0][0] = new Kart('G');
        kartlar[0][1] = new Kart('C');
        kartlar[0][2] = new Kart('E');
        kartlar[0][3] = new Kart('D');
        kartlar[1][0] = new Kart('B');
        kartlar[1][1] = new Kart('A');
        kartlar[1][2] = new Kart('F');
        kartlar[1][3] = new Kart('D');
        kartlar[2][0] = new Kart('C');
        kartlar[2][1] = new Kart('E');
        kartlar[2][2] = new Kart('A');
        kartlar[2][3] = new Kart('H');
        kartlar[3][0] = new Kart('G');
        kartlar[3][1] = new Kart('B');
        kartlar[3][2] = new Kart('H');
        kartlar[3][3] = new Kart('F');
        case 3:
        kartlar[0][0] = new Kart('B');
        kartlar[0][1] = new Kart('D');
        kartlar[0][2] = new Kart('E');
        kartlar[0][3] = new Kart('C');
        kartlar[1][0] = new Kart('D');
        kartlar[1][1] = new Kart('B');
        kartlar[1][2] = new Kart('A');
        kartlar[1][3] = new Kart('H');
        kartlar[2][0] = new Kart('G');
        kartlar[2][1] = new Kart('E');
        kartlar[2][2] = new Kart('C');
        kartlar[2][3] = new Kart('F');
        kartlar[3][0] = new Kart('F');
        kartlar[3][1] = new Kart('G');
        kartlar[3][2] = new Kart('H');
        kartlar[3][3] = new Kart('A');
        case 4:
        kartlar[0][0] = new Kart('C');
        kartlar[0][1] = new Kart('H');
        kartlar[0][2] = new Kart('B');
        kartlar[0][3] = new Kart('H');
        kartlar[1][0] = new Kart('G');
        kartlar[1][1] = new Kart('A');
        kartlar[1][2] = new Kart('C');
        kartlar[1][3] = new Kart('B');
        kartlar[2][0] = new Kart('F');
        kartlar[2][1] = new Kart('A');
        kartlar[2][2] = new Kart('E');
        kartlar[2][3] = new Kart('D');
        kartlar[3][0] = new Kart('F');
        kartlar[3][1] = new Kart('E');
        kartlar[3][2] = new Kart('G');
        kartlar[3][3] = new Kart('D');
        case 5:
        kartlar[0][0] = new Kart('E');
        kartlar[0][1] = new Kart('D');
        kartlar[0][2] = new Kart('H');
        kartlar[0][3] = new Kart('D');
        kartlar[1][0] = new Kart('C');
        kartlar[1][1] = new Kart('H');
        kartlar[1][2] = new Kart('F');
        kartlar[1][3] = new Kart('B');
        kartlar[2][0] = new Kart('E');
        kartlar[2][1] = new Kart('A');
        kartlar[2][2] = new Kart('F');
        kartlar[2][3] = new Kart('G');
        kartlar[3][0] = new Kart('A');
        kartlar[3][1] = new Kart('C');
        kartlar[3][2] = new Kart('G');
        kartlar[3][3] = new Kart('B');
        }
        while(oyunBittiMi() == false){
        tahminEt();
        }
        if(oyunBittiMi() == true){
        seviye++;
        }
        System.out.println("Tebrikler! "+seviye+". seviyeyi geçtiniz.");
       
        }
    }}
